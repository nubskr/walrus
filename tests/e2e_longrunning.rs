...
fn e2e_massive_data_throughput_test() {
    ...
    println!("  Bytes written: {} ({:.2} MB)", bytes_written, bytes_written as f64 / 1_000_000.0);
    println!("  Bytes read: {} ({:.2} MB)", bytes_read, bytes_read as f64 / 1_000_000.0);
    println!("  Entries written: {}", entries_written);
    println!("  Entries read: {}", entries_read);
    println!("  Validation errors: {}", validation_errors);
    println!("  Write throughput: {:.2} MB/s", (bytes_written as f64 / 1_000_000.0) / elapsed.as_secs_f64());
    println!("  Read throughput: {:.2} MB/s", (bytes_read as f64 / 1_000_000.0) / elapsed.as_secs_f64());
