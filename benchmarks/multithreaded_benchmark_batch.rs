...
fn parse_batch_size() -> usize {
    ...
    // Default batch size (halved for smaller batches)
    450
}
...
fn print_usage() {
    ...
    println!("  <number>     Number of entries per batch (1-10000000, default: 450)");
    println!();
...
fn multithreaded_batch_benchmark() {
    ...
    println!(
        "Configuration: 5 threads, {:.0}s write phase only, batch size: {} entries/batch",
    ...
    let num_threads = 5; // Reduced to 5 threads with smaller entries
