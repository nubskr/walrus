...
fn multithreaded_benchmark() {
    ...
    writeln!(csv_file, "timestamp,elapsed_seconds,writes_per_second,bytes_per_second,total_writes,total_bytes").expect("Failed to write CSV header");
    ...
    let monitor_handle = thread::spawn(move || {
        ...
        loop {
            ...
            if should_log {
                ...
                writeln!(csv_file, "{},{:.2},{:.0},{:.0},{},{}", 
                    timestamp, elapsed_total, writes_per_second, bytes_per_second, current_writes, current_bytes)
                    .expect("Failed to write to CSV");
                csv_file.flush().expect("Failed to flush CSV");
                
                // Print progress only if there's activity
                if current_writes > last_writes {
                    println!("[Monitor] {:.1}s: {:.0} writes/sec, {:.2} MB/sec, total: {} writes", 
                        elapsed_total, writes_per_second, bytes_per_second / (1024.0 * 1024.0), current_writes);
                }
            }
    ...
    for thread_id in 0..num_threads {
        ...
        let handle = thread::spawn(move || {
            ...
            println!("Thread {} ({}): {} writes, {} KB, {} errors", 
                thread_id, topic, local_writes, local_write_bytes / 1024, local_errors);
    ...
    println!("Write Throughput: {:.0} ops/sec", final_writes as f64 / write_elapsed.as_secs_f64());
    println!("Write Bandwidth: {:.2} MB/sec", (final_write_bytes as f64 / (1024.0 * 1024.0)) / write_elapsed.as_secs_f64());
    println!();
